package com.fairytale.fortunetarot.widget.wheel;
...
import com.fairytale.fortunetarot.widget.wheel.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	...
	// Shadows drawables
	//private GradientDrawable topShadow; // 绘制图片的渐变色
	...
	protected void onDraw(Canvas canvas) {
		...
		if (viewAdapter != null && viewAdapter.getItemsCount() > 0) {
			...
			for (int i = 0; i < count; i++) {
				View textView = (TextView) itemsLayout.getChildAt(i);
				if ((textView instanceof TextView)) {
					if (currentItem - firstItem == i) {
//						((TextView) textView).setTextColor(Color.rgb(51,51,51));
						((TextView) textView).setTextColor(getContext().getResources().getColor(R.color.text_title_white));
					} else {
						((TextView) textView)
...
//		bottomShadow.draw(canvas);
	...
	private void drawCenterRect(Canvas canvas) {
		if (paint == null) {
			...
			paint.setColor(getContext().getResources().getColor(R.color.line_light_gray));// 设置为红笔
			paint.setAntiAlias(true);// 锯齿不显示
		}
		int center = getHeight() / 2;
		int offset = (int) (getItemHeight() / 2 * 1.2);
		if (isShow) {
			canvas.drawLine(0, center - offset, getWidth(), center - offset, paint);//画线
			canvas.drawLine(0, center + offset, getWidth(), center + offset, paint);//画线
		}
